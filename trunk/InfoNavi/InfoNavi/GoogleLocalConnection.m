...
-(void)getGoogleObjectsWithQuery:(NSString *)query andMapRegion:(MKCoordinateRegion)region andNumberOfResults:(int)numResults addressesOnly:(BOOL)addressOnly andReferer:(NSString *)referer;
{
	...
	NSURL *googleLocationURL = [NSURL URLWithString:[NSString stringWithFormat:@"http://ajax.googleapis.com/ajax/services/search/local?v=1.0&mrt=localonly&q=%@&rsz=%@&start=0&sll=%f,%f&sspn=%f,%f",query,numberOfResults,centerLat,centerLng,spanLat,spanLng]];
    
	NSMutableURLRequest *googleRequest = [NSMutableURLRequest requestWithURL:googleLocationURL cachePolicy:NSURLRequestUseProtocolCachePolicy timeoutInterval:10];
	[googleRequest setValue:@"gzip" forHTTPHeaderField:@"Accept-Encoding"];
	[googleRequest setValue:referer forHTTPHeaderField:@"Referer"];

	[self cancelGetGoogleObjects];
	
	connection = [[NSURLConnection alloc] initWithRequest:googleRequest delegate:self startImmediately:YES];
	if (connection) {
